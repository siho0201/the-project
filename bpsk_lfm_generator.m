...
bpsk_bandwidth = 300;   % BPSK의 Bandwidth

sirs_dB = 0 : 1 : 30;

% Check if the folder exists
if ~exist('D:/Documents/Overlapped_Signal/data3', 'dir')
   % If not, create the folder
   mkdir('D:/Documents/Overlapped_Signal/data3');
end

for sir_dB = sirs_dB
    sir = 10 ^ (sir_dB / 10);
    bits = zeros(300, sample/symbol_sample);
    bpsk_iq = zeros(300, sample);
    ...
    for i = 1 : 300
        ...
        % 신호 생성
        bit = randi([0 1], 1, sample_ex/symbol_sample); % random bits
        phase = pi*randn(1, sample_ex); % random phase
        bpsk_signal = bpsk(bit, t_ex, phase, symbol_duration, bpsk_bandwidth); % bpsk signal
        bpsk_signal = bpsk_signal(1 : sample);
        bit = bit(1 : sample/symbol_sample);
        phase = phase(1 : sample);
        power_bpsk = sum(abs(bpsk_signal) .^ 2); % bpsk power
        ...
        lfm_signal = sqrt(double(sir) * power_bpsk  / power_lfm) * lfm_sig; % sir 고려한 lfm signal
        overlap_signal = bpsk_signal + lfm_signal; % 합성 신호
        ...
        bpsk_img = stft(bpsk_signal, fs, 'Window', window, 'OverlapLength', overlap, 'FFTLength', nfft);
    ...
    filedir = "D:/Documents/Overlapped_Signal/data3/" + filename;
    save(filedir, 'bpsk_iq', 'lfm_iq', 'overlap_iq', 'bpsk_stft', 'lfm_stft', 'overlap_stft', 'bits', 'phases', 'bpsk_bandwidth', 'lfm_bandwidth', 'sir_dB', 'lfm_chirp_duration');
end