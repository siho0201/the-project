...
clc
clear
load('pred_label_iq_fsk4_complex.mat')
load('merged_data_and_bits_f.mat')
...
for number = 1:465
    ...
    sample_iq_re = istft(squeeze(restored_stft(number,:,:)), fs, 'Window', window, 'OverlapLength', overlap, 'FFTLength', nfft);
    % sample_iq_re = sample_iq.*exp(-1j*phase);
    bits = merged_bits(number,:);
    
    % 4개의 샘플마다 하나의 비트로 변환합니다.

    
    converted_bits = fskdemod(sample_iq_re , M, freqsep, 4, fs).';
