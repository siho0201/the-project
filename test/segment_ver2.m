...
save('merged_data_and_bits.mat', 'merged_data', 'merged_bits','merged_bpsk','merged_lfm','total_data');
%% step 2
clc
clear
load('pred_label_iq_bpsk_ver2.mat')
...
samp_amp = squeeze(Amplitude);
samp_phase = squeeze(Phase);

% samp_amp(samp_amp <0.01) =0;

stft_IQ = samp_amp .* exp(1i * samp_phase);
restored_stft = stft_IQ;
